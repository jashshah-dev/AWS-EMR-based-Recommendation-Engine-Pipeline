#!/bin/bash -xe
upload_log() {
  aws s3 cp /tmp/userdata_execution.log s3://airflowbucketsetup/logs/
}
...
airflow db init
airflow users create -u airflow -f airflow -l airflow -r Admin -e airflow@gmail.com -p root
mkdir /home/ubuntu/dags
aws s3 cp s3://airflowbucketsetup/dags/airflow_dag.py /home/ubuntu/dags
